...
public class ConstsClassifyServiceImpl implements IConstsClassifyService {
	...
	@Override
	public TailPage<ConstsClassify> searchPage(ConstsClassify searchEntity, TailPage<ConstsClassify> page) {
		Integer itemsTotalCount=entityDao.getTotalItemsCount(searchEntity);
        List<ConstsClassify> items=entityDao.searchPage(searchEntity,page);
        page.setItemsTotalCount(itemsTotalCount);
        page.setItems(items);
		return page;
	}
