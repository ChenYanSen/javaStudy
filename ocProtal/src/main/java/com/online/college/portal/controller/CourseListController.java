...
public class CourseListController {
    ...
    public ModelAndView search(String c, String sort, TailPage<Course> page) {
       ...
       /* String curCode = "-1";//当前方向code
        ...
        mv.addObject("curSubCode", curSubCode);*/
        Course queryEntity=new Course();
        queryEntity.setName(c);
      /*  if (!"-1".equals(curCode)) {
            searchEntity.setClassify(curCode);
        }
        if (!"-2".equals(curSubCode)) {
            searchEntity.setSubClassify(curSubCode);
        }*/
        //排序参数
        if ("pop".equals(sort)) {//最    热
            page.descSortField("studyCount");
        } else {
            sort = "last";
            page.descSortField("id");
        ...
        //分页参数
        queryEntity.setOnsale(CourseEnum.ONSALE.value());
        page = this.courseService.searchPage(queryEntity, page);
        mv.addObject("page", page);
        return mv;
    }
}
